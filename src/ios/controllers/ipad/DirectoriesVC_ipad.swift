...
final class DirectoriesVCIPAD: NYXViewController {
	...
	// Logger
	private let logger = Logger(label: "logger.DirectoriesVCIPAD")
	...
	private func toggleCoverImage() {
		if hasImage {
			...
			tableView.frame = CGRect(view.x, imageView.maxY + 10, view.width, view.height - (imageView.maxY + 10))
...
extension DirectoriesVCIPAD: UITableViewDataSource {
	func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
		handleEmptyView(tableView: tableView, isEmpty: songs.isEmpty)
		return songs.count
	}
...
extension DirectoriesVCIPAD: UITableViewDelegate {
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		let item = songs[indexPath.row]
		if item.type == .directory {
			let newPath = self.path == nil ? item.name : self.path! + "/" + item.name
			let vc = DirectoriesVCIPAD(mpdBridge: mpdBridge, path: newPath)
...
extension DirectoriesVCIPAD: Themed {
	func applyTheme(_ theme: Theme) {
	}
}

// MARK: - UIAdaptivePresentationControllerDelegate
extension DirectoriesVCIPAD: UIAdaptivePresentationControllerDelegate {
