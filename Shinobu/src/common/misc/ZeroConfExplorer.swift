...
extension ZeroConfExplorer: NetServiceBrowserDelegate {
	...
	func netServiceBrowser(_ browser: NetServiceBrowser, didFind service: NetService, moreComing: Bool) {
		let mpdServer = MPDServer(hostname: "", port: 0, password: "", name: service.name)
		services[service] = mpdServer
	...
	func netServiceDidResolveAddress(_ sender: NetService) {
		...
		for addressBytes in addresses where found == false {
			...
			if found {
				...
				#endif
				let server = MPDServer(hostname: tmpIP, port: UInt16(sender.port), password: "", name: sender.name)
